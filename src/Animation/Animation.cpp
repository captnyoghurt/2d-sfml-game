...
Animation::Animation(
    int refreshTime, const std::vector<sf::Rect<short>>& rects,
    std::list<std::pair<ManageSurfaces::e_thing,
                        std::shared_ptr<Surface>>>::iterator surf)
    : m_rectSprites(rects) {
...
Animation::Animation(
    ...
    std::list<std::pair<ManageSurfaces::e_thing,
                        std::shared_ptr<Surface>>>::iterator surf,
    int endX, int endY, int manDist, int loop, bool killWhenDone)
    : m_rectSprites(rects) {
  m_refreshTime = refreshTime;
  m_rectAct = 0;
  m_surface = surf;
...
bool Animation::getMoving() const { return m_moving; }

// Return if the animation is finished
bool Animation::getFinished() const { return m_finished; }

// Return if the animation is supposed to be killed when its done
bool Animation::getKillWhenDone() const { return m_killWhenDone; }

// Return the remaining number of loop
int Animation::getNbLoop() const { return m_nbLoop; }

// Return the animation refresh time
int Animation::getRefreshTime() const { return m_refreshTime; }

// Return the distance walked by the animation at each call
int Animation::getManathanDistance() const { return m_manathanDistance; }

// Return the destination position X
int Animation::getEndX() const { return m_endX; }

// Return the destination position Y
int Animation::getEndY() const { return m_endY; }
...
int Animation::setRefreshTime(const int& t) {
  if (t <= 0) THROW_VALUE(std::to_string(t));
...
int Animation::update() {
  ...
  if (m_clock.getElapsedTime().asMilliseconds() >= m_refreshTime) {
    ...
    if ((unsigned)m_rectAct >= m_rectSprites.size()) {
      ...
      if (m_nbLoop == 0) {
        m_finished = true;
        std::dynamic_pointer_cast<SurfaceSprite>(m_surface->second)
            ->setTextureRect(sf::IntRect(
                m_rectSprites.at(0).left, m_rectSprites.at(0).top,
                m_rectSprites.at(0).width, m_rectSprites.at(0).height));
        return 1;
      }
    }
    std::dynamic_pointer_cast<SurfaceSprite>(m_surface->second)
        ->setTextureRect(sf::IntRect(m_rectSprites.at(m_rectAct).left,
                                     m_rectSprites.at(m_rectAct).top,
                                     m_rectSprites.at(m_rectAct).width,
                                     m_rectSprites.at(m_rectAct).height));
    m_clock.restart();
    return 1;
  }
...
// Start the animation
int Animation::start(int endX, int endY, int manDist, int loop,
                     bool killWhenDone) {
  m_finished = false;
  m_endX = endX;
  m_endY = endY;
