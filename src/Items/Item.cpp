...
Item::Item(const int &id, const std::string &name,
           const std::string &description, const int &priceBuy,
           const int &priceSell, const int &iconId,
           const Item::e_ItemType &type)
    : m_id(id),
      m_name(name),
...
int Item::getPriceBuy() const { return m_priceBuy; }

// Return the price when sold
int Item::getPriceSell() const { return m_priceSell; }

// Modify the icon id of the item
int Item::getIconId() const { return m_iconId; }

// Return the type of the item
Item::e_ItemType Item::getType() const { return m_type; }

// Return the effects
std::deque<BattleEffects> Item::getEffects() const { return m_effects; }

// Return the effects with modifying possibilities
std::deque<BattleEffects> &Item::getRealEffects() { return m_effects; }

// Modify the id
int Item::setId(const int &id) {
  if (id < 0) THROW_VALUE("Impossible id : " + std::to_string(id));
...
int Item::setPriceBuy(const int &p) {
  if (p < 0) THROW_VALUE("Impossible price : " + std::to_string(p));
...
int Item::setIconId(const int &id) {
  if (!((unsigned)id <
        (unsigned)ICONS_SURFACE_BY_COLUMN * ICONS_SURFACE_BY_LINE))
