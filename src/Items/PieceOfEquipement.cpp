...
PieceOfEquipement::PieceOfEquipement() : Item() {
  m_position = PieceOfEquipement::e_POEPosition::POE_TOTAL;
  m_mateId = TeamMate::e_MateId::MATE_ID_ALL;
  m_equiped = false;
}

PieceOfEquipement::PieceOfEquipement(const Item &item, const e_POEPosition &pos,
...
TeamMate::e_MateId PieceOfEquipement::getMateId() const { return m_mateId; }

// Return if the piece is equiped
bool PieceOfEquipement::getEquiped() const { return m_equiped; }

// Modify the position of the piece of equipement
int PieceOfEquipement::setPosition(
    const PieceOfEquipement::e_POEPosition &pos) {
  if (pos < 0 || pos > PieceOfEquipement::e_POEPosition::POE_TOTAL)
    THROW_VALUE("Impossible value of equipement position : " +
                std::to_string(pos));
