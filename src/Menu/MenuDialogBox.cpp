...
MenuDialogBox::MenuDialogBox(ManageRessources &ress, ManageSurfaces &surf,
                             int lastEventLayer)
    : Menu(ress, surf, lastEventLayer),
      m_textStream(std::ios_base::app | std::ios_base::in |
                   std::ios_base::out) {
  m_cursorSurfaceDown = surf.addSurface(ManageSurfaces::e_thing::SPRITE,
                                        std::make_shared<SurfaceSprite>());

  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
      ->setTexture(ress.getTheTexture(0));
  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
...
std::string MenuDialogBox::getText() const { return m_text; }
...
MenuDialogBox::getCursorSurfaceDown() {
...
int MenuDialogBox::setText(const std::string &text) {
  if (m_initialized) THROW_INIT("Dialog box already initiate : " + text);
...
int MenuDialogBox::continueText(ManageRessources &ress, ManageSurfaces &surf) {
  ...
  if (m_textStream.eof()) return 1;

  m_textSurface.push_back(surf.addSurface(ManageSurfaces::e_thing::TEXT,
                                          std::make_shared<SurfaceText>()));
  m_textSurface.back()->second->setDimensions(
      m_x + MENUS_BORDER_X, m_y + MENUS_BORDER_Y,
  ...
  while (fillHeightCounter < m_height) {
      ...
      if (!(lineActual.empty() && c == ' ')) {
        if (c == '\\') {
          ...
          if (heightSup > 0) {
            ...
            remakeText(lineActual,
                       std::dynamic_pointer_cast<SurfaceText>(
                           m_textSurface.at(m_textSurface.size() - 2)->second),
                       ress);
            break;
          }
        } else {
    ...
    if (fillWidthCounter >= MENU_DIALOG_BOX_CHARACTER_BY_LINE) {
      if (!m_textStream.eof() &&
              (lineActual.back() >= 'a' && lineActual.back() <= 'z') ||
          (lineActual.back() >= 'A' && lineActual.back() <= 'Z')) {
        m_textStream.get(c);
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
      ...
      remakeText(
          lineActual,
          std::dynamic_pointer_cast<SurfaceText>(m_textSurface.back()->second),
          ress);
  ...
  if (m_textStream.eof()) m_cursorSurfaceDown->second->setEnable(false);

  return 0;
}

// Load the menu
int MenuDialogBox::load(ManageRessources &ress, ManageSurfaces &surf,
                        const std::string &text, const int &xcam,
                        const int &ycam, const int &x, const int &y,
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setDimensions(m_x, m_y, m_width, m_height);
...
// Update surfaces
int MenuDialogBox::update(Game &g) { return 0; }
...
int MenuDialogBox::remakeText(const std::string &str,
                              std::shared_ptr<SurfaceText> text,
...
int MenuDialogBox::newLine(ManageSurfaces &surf) {
  m_textSurface.push_back(surf.addSurface(ManageSurfaces::e_thing::TEXT,
                                          std::make_shared<SurfaceText>()));
  m_textSurface.back()->second->setDimensions(
      m_textSurface.at(m_textSurface.size() - 2)->second->getX(),
      m_textSurface.at(m_textSurface.size() - 2)->second->getY() +
  ...
  std::dynamic_pointer_cast<SurfaceText>(m_textSurface.back()->second)
      ->setCharacterSize(MENU_DIALOG_BOX_FONTSIZE);
