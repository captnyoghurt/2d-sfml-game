...
Menu::Menu(ManageRessources &ress, ManageSurfaces &surf, int lastEventLayer) {
  m_background = surf.addSurface(ManageSurfaces::e_thing::SPRITE,
                                 std::make_shared<SurfaceSprite>());
  /*if(showCursor)
          m_cursorSurface = surf.addSurface(ManageSurfaces::e_thing::SPRITE,
  std::shared_ptr<Surface>(new SurfaceSprite)); m_selectedChoiceSurface =
  surf.addSurface(ManageSurfaces::e_thing::SPRITE, std::shared_ptr<Surface>(new
  SurfaceSprite));*/
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setTextureRect(sf::IntRect(MENU_SURFACE_BEGIN_X, MENU_SURFACE_BEGIN_Y,
                                   MENU_SURFACE_WIDTH, MENU_SURFACE_HEIGHT));
  /*if (showCursor)
  {
...
bool Menu::getShown() const { return m_shown; }

// Return if the menu is initialized
bool Menu::getInitialized() const { return m_initialized; }
...
std::vector<std::list<std::pair<ManageSurfaces::e_thing,
std::shared_ptr<Surface>>>::iterator> Menu::getChoicesSurface()
{
...
std::shared_ptr<Surface>>>::iterator Menu::getCursorSurface()
{
...
std::list<std::pair<ManageSurfaces::e_thing,
std::shared_ptr<Surface>>>::iterator Menu::getSelectedChoiceSurface()
{
...
int Menu::getX() const { return m_x; }

// Return the position Y of the menu
int Menu::getY() const { return m_y; }

// Return the width of the menu
int Menu::getWidth() const { return m_width; }

// Return the height of the menu
int Menu::getHeight() const { return m_height; }

// Modify if the menu should be shown or not
int Menu::setShown(const bool &b) {
  if (!m_initialized) THROW_INIT("Shown " + std::to_string(b));

  if (m_shown == b) return 0;
...
// Load the menu
int Menu::load(ManageRessources &ress, ManageSurfaces &surf, const int &x,
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setEnable(true);
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setScale(float(w) / (MENU_SURFACE_WIDTH),
                 float(h) / (MENU_SURFACE_HEIGHT));
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
  ...
  {
          m_choicesSurface.at(i) =
  surf.addSurface(ManageSurfaces::e_thing::TEXT, std::shared_ptr<Surface>(new
  SurfaceText));
          ...
          std::dynamic_pointer_cast<SurfaceText>(m_choicesSurface.at(i)->second)->setPosition(float(x),
  float(y));
  }

  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)->setTextureRect(sf::IntRect(x,
  y, w, h));*/
  return 0;
}
