...
MenuChoice::MenuChoice(ManageRessources &ress, ManageSurfaces &surf,
                       int lastEventLayer)
    : Menu(ress, surf, lastEventLayer) {
  m_selectedChoiceSurface =
      surf.addSurface(ManageSurfaces::e_thing::SPRITE,
                      std::shared_ptr<Surface>(new SurfaceSprite));
  m_cursorSurfaceDown =
      surf.addSurface(ManageSurfaces::e_thing::SPRITE,
                      std::shared_ptr<Surface>(new SurfaceSprite));
  m_cursorSurfaceUp =
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceUp->second)
      ->setTexture(ress.getTheTexture(RESSOURCE_TEXTURE_NUMBER_MENU));
  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceUp->second)
      ->setTextureRect(sf::IntRect(
          MENU_SURFACE_CURSOR_UP_BEGIN_X, MENU_SURFACE_CURSOR_UP_BEGIN_Y,
          MENU_SURFACE_CURSOR_UP_WIDTH, MENU_SURFACE_CURSOR_UP_HEIGHT));
  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
      ->setTexture(ress.getTheTexture(RESSOURCE_TEXTURE_NUMBER_MENU));
  std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
      ->setTextureRect(sf::IntRect(
          MENU_SURFACE_CURSOR_DOWN_BEGIN_X, MENU_SURFACE_CURSOR_DOWN_BEGIN_Y,
          MENU_SURFACE_CURSOR_DOWN_WIDTH, MENU_SURFACE_CURSOR_DOWN_HEIGHT));
  std::dynamic_pointer_cast<SurfaceSprite>(m_selectedChoiceSurface->second)
      ->setTexture(ress.getTheTexture(RESSOURCE_TEXTURE_NUMBER_MENU));
  std::dynamic_pointer_cast<SurfaceSprite>(m_selectedChoiceSurface->second)
      ->setTextureRect(sf::IntRect(
          MENU_SURFACE_SELECTED_BEGIN_X, MENU_SURFACE_SELECTED_BEGIN_Y,
          MENU_SURFACE_SELECTED_WIDTH, MENU_SURFACE_SELECTED_HEIGHT));
...
MenuChoice::getCursorSurfaceDown() {
...
std::list<
    std::pair<ManageSurfaces::e_thing, std::shared_ptr<Surface>>>::iterator
MenuChoice::getSelectedChoiceSurface() {
  return m_selectedChoiceSurface;
}

// Return the selected choice
int MenuChoice::getSelectedChoice() const { return m_selectedChoice; }

// Return the first choice actually shown
int MenuChoice::getFirstChoiceShown() const { return m_firstChoiceShown; }

// Return the last choice actually shown
int MenuChoice::getLastChoiceShown() const { return m_lastChoiceShown; }
...
int MenuChoice::setShown(const bool &b) {
  if (Menu::setShown(b) >= 0) {
    ...
    if (b) setSelectedChoice(m_selectedChoice);
...
int MenuChoice::setSelectedChoice(const int &ch) {
  ...
  m_cursorSurfaceDown->second->setEnable((unsigned)m_lastChoiceShown <
                                         m_choices.size() - 1);
  ...
  /*std::dynamic_pointer_cast<SurfaceSprite>(m_selectedChoiceSurface->second)->setDimensions(std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)->getX(),
          std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)->getY()
     + ((m_selectedChoice - m_firstChoiceShown)*MENUS_GAP_BETWEEN_LINES) +
     MENUS_BORDER_Y - ((MENU_SURFACE_SELECTED_HEIGHT - MENU_SIMPLE_FONTSIZE) /
     4),
  ...
  for (unsigned int i(0); i < m_choices.size(); i++) {
    if (i >= (unsigned)m_firstChoiceShown && i <= (unsigned)m_lastChoiceShown) {
      m_choices.at(i)->setShown(true, 0,
                                -m_firstChoiceShown * MENUS_GAP_BETWEEN_LINES);
    } else {
  ...
  m_selectedChoiceSurface->second->setDimensions(
      ...
      m_choices.at(m_selectedChoice)->getY() -
          m_firstChoiceShown * MENUS_GAP_BETWEEN_LINES,
      int(std::dynamic_pointer_cast<SurfaceSprite>(
              m_selectedChoiceSurface->second)
              ->getGlobalBounds()
              .width),
      int(std::dynamic_pointer_cast<SurfaceSprite>(
              m_selectedChoiceSurface->second)
...
int MenuChoice::resizeChoiceSurface() {
  ...
  w = (float)m_choices.at(m_selectedChoice)
          ->getRealSurface()
          ->second->getWidth() +
      2 * MENUS_BORDER_X;
  h = (float)m_choices.at(m_selectedChoice)
          ->getRealSurface()
          ->second->getHeight();
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_selectedChoiceSurface->second)
      ->scale(w / std::dynamic_pointer_cast<SurfaceSprite>(
                      m_selectedChoiceSurface->second)
                      ->getGlobalBounds()
                      .width,
              h / std::dynamic_pointer_cast<SurfaceSprite>(
                      m_selectedChoiceSurface->second)
                      ->getGlobalBounds()
                      .height);
  return 0;
...
// Load the menu
int MenuChoice::loadFromStream(ManageRessources &ress, ManageSurfaces &surf,
                               std::stringstream &ss, const int &xcam,
                               const int &ycam, const int &wchoice,
                               const int &hchoice) {
  ...
  while (
      ...
      ((firstLine.size() > 0 && firstLine[0] == '#') || firstLine.size() == 0))
    ;

  if (firstLine == "AUTO") autoPlacement = true;

  ss >> m_lastChoiceShown >> m_x >> m_y >> m_width >> m_height;
  std::getline(ss, firstLine);

  // Place the cursors for the scrolling menu
  m_cursorSurfaceUp->second->setDimensions(
      xcam + m_x + (m_width / 2) -
          int(std::dynamic_pointer_cast<SurfaceSprite>(
                  m_cursorSurfaceUp->second)
                  ->getGlobalBounds()
                  .width),
      ycam + m_y,
      int(std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceUp->second)
              ->getGlobalBounds()
              .width),
      int(std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceUp->second)
              ->getGlobalBounds()
              .height));
  m_cursorSurfaceDown->second->setDimensions(
      xcam + m_x + (m_width / 2) -
          int(std::dynamic_pointer_cast<SurfaceSprite>(
                  m_cursorSurfaceDown->second)
                  ->getGlobalBounds()
                  .width),
      ycam + m_y + m_height -
          int(std::dynamic_pointer_cast<SurfaceSprite>(
                  ...
                  ->getGlobalBounds()
                  .height),
      int(std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
              ->getGlobalBounds()
              .width),
      int(std::dynamic_pointer_cast<SurfaceSprite>(m_cursorSurfaceDown->second)
              ->getGlobalBounds()
  ...
  while (std::getline(ss, firstLine) && std::getline(ss, secondLine)) {
    ...
    if (autoPlacement) {
      ...
      cy = ycam + m_y + MENUS_BORDER_Y +
           (MENUS_GAP_BETWEEN_LINES * numberOfChoices);
    } else {
    ...
    m_choices.back()->load(surf, secondLine, f, cx, cy, true, act, wchoice,
                           hchoice);
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setEnable(true);
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setScale(float(m_width) / (MENU_SURFACE_WIDTH),
                 float(m_height) / (MENU_SURFACE_HEIGHT));
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setDimensions(xcam + m_x, ycam + m_y, m_width, m_height);
  m_initialized = true;
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_selectedChoiceSurface->second)
      ->setScale(float(m_width) / (MENU_SURFACE_SELECTED_WIDTH),
                 float(std::dynamic_pointer_cast<SurfaceSprite>(
                           m_choices.at(0)->getRealSurface()->second)
                           ->getGlobalBounds()
                           .height) /
                     (MENU_SURFACE_SELECTED_HEIGHT));

  setSelectedChoice(0);

  return 0;
}
int MenuChoice::loadFromFile(ManageRessources &ress, ManageSurfaces &surf,
  ...
  if (!file.is_open()) THROW_RESSOURCE("File for menu choice", filename);
...
int MenuChoice::update(Game &g) {
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setPosition(float(x + m_x), float(y + m_y));

  for (unsigned int i(0); i < m_choices.size(); i++) {
    std::dynamic_pointer_cast<SurfaceSprite>(
        m_choices.at(i)->getSurface()->second)
        ->setPosition(
            float(x + m_x + MENUS_BORDER_X),
            float(x + m_y + MENUS_BORDER_Y + (i * MENUS_GAP_BETWEEN_LINES)));
  }
