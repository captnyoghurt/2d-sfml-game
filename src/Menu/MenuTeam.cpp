...
MenuTeam::MenuTeam(ManageRessources &ress, ManageSurfaces &surf,
                   int lastEventLayer)
...
MenuTeam::e_MenuTeamType MenuTeam::getType() const { return m_type; }
...
// brief Load the menu
int MenuTeam::load(const e_MenuTeamType &type, TeamBattle &tb,
                   ManageRessources &ress, ManageSurfaces &surf,
                   const int &xcam, const int &ycam, const int &wchoice,
                   const int &hchoice) {
  if (tb.getTeam().empty()) {
  ...
  std::stringstream teamStringStream(makeChoicesSS(tb));
  int ret = MenuChoice::loadFromStream(ress, surf, teamStringStream, xcam, ycam,
                                       wchoice, hchoice);
...
std::stringstream MenuTeam::makeChoicesSS(TeamBattle &tb) {
  ...
  ss << "AUTO\n"
     ...
     << MENU_TEAM_X << " " << MENU_TEAM_Y << " " << MENU_TEAM_WIDTH << " "
     << MENU_TEAM_HEIGHT << "\n";

  for (unsigned int i(0); i < tb.getTeam().size(); i++) {
    ss << "NOTHING\n" << tb.getTeam().at(i)->getName();

    if ((unsigned)(i + 1) < tb.getTeam().size()) ss << "\n";
  }
  ...
  for (unsigned int i(0); i < tb.getRealTeam().size(); i++) {
    switch (m_type) {
      case MenuTeam::MENU_TEAM_SPELLS: {
        IG_Action actionOpenMenuSpells(std::bind(&af_openMenuSpells,
                                                 *tb.getRealTeam().at(i),
                                                 std::placeholders::_1));
        this->getChoices().at(i)->setAction(actionOpenMenuSpells);
      } break;
