...
#include "../Map/Physic/ManageHitbox.h"
#include "Character.h"
...
class Team {
 ...
 public:
   ...
   * \return int object.
   */
  int setDimension(const unsigned short &x, const unsigned short &y,
   ...
   * \return std::list<std::pair<ManageSurfaces::e_thing,
   * std::shared_ptr<Surface>>>::iterator object.
   */
  std::list<
      std::pair<ManageSurfaces::e_thing, std::shared_ptr<Surface>>>::iterator
  setWalkStand(std::list<std::pair<ManageSurfaces::e_thing,
                                   std::shared_ptr<Surface>>>::iterator l);

  // Move the team
  int move(const short &dx, const short &dy,
           e_teamDirection direction = UNKNOWN);
  int stopMoving(Team::e_teamDirection dir = UNKNOWN);
  // int moveWithCamera(const short &dx, const short &dy, Camera &c,
  // e_teamDirection direction = UNKNOWN);
  // Update the movement
 ...
 private:
  ...
  std::list<std::pair<ManageSurfaces::e_thing,
                      std::shared_ptr<Surface>>>::iterator m_walkStand;
