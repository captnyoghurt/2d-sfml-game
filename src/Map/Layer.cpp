...
vector<vector<Tile> > Layer::getLayer() const { return m_layer; }

// Return the width of the layer
int Layer::getWitdth() const { return m_width; }

// Return the height of the layer
int Layer::getHeight() const { return m_height; }

// Return the width of each tile
int Layer::getTileW() const { return m_tileW; }

// Return the height of each tile
int Layer::getTileH() const { return m_tileH; }

// Return the tile asked (x,y) from the layer
Tile &Layer::getTile(int x, int y) { return m_layer.at(x).at(y); }
...
int Layer::resize(const int &w, const int &h) {
  if (w < 0 || h < 0) THROW_VALUE(std::to_string(w) + "/" + std::to_string(h));
...
int Layer::printLayer(Texture tileset, RenderWindow &win, int xCam, int yCam,
                      int wCam, int hCam) {
  Sprite sp;
  ...
  for (int i(0); i < wCam / m_tileW + 2 && i < m_width && i + iBegin < m_width;
       i++) {
    if (i + iBegin >= 0) {
      for (int j(0);
           j < hCam / m_tileH + 2 && j < m_height && j + jBegin < m_height;
           j++) {
        if (j + jBegin >= 0) {
          // The Most top-left Tile that can be seen (even partly) with the
          // camera
          Tile t(m_layer.at(i + iBegin).at(j + jBegin));

          if (!t.getEmpty()) {
            sp.setPosition(float((i - 1) * m_tileW - xCam % m_tileW),
                           float((j - 1) * m_tileH - yCam % m_tileH));
            sp.setTextureRect(
                sf::IntRect(t.getX(), t.getY(), t.getWidth(), t.getHeight()));
            win.draw(sp);
          }
        }
      }
