...
ManageSurfaces::getSurfaces() const {
  return m_surfaces;
}

// Return modifying element
std::pair<ManageSurfaces::e_thing, std::shared_ptr<Surface>>
    &ManageSurfaces::getModifyingSurface(int n) {
  int i(0);
  if (m_surfaces.size() >= (unsigned)n) return *m_surfaces.begin();
...
ManageSurfaces::addSurface(ManageSurfaces::e_thing type,
...
// Delete an element
int ManageSurfaces::deleteSurface(
      ...
      switch (it->first) {
        case TEXT:
          std::dynamic_pointer_cast<SurfaceText>(it->second)
              ->setPosition(float(it->second->getX() - cam.getX()),
                            float(it->second->getY() - cam.getY()));
        ...
        case SPRITE:
          ...
          break;
        default:
          THROW_VALUE("Error when blitting a surface " +
