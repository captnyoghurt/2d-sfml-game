...
Camera::Camera(const int &x, const int &y, const int &w, const int &h,
               const int &mx, const int &my, bool blocked) {
  m_blocked = blocked;
  m_x = x;
  m_y = y;
  m_width = w;
...
bool Camera::getBlocked() const { return m_blocked; }
...
int Camera::getMaxX() const { return m_maxX; }

// Return the maxY of the camera
int Camera::getMaxY() const { return m_maxY; }
