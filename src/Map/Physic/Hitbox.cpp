...
Hitbox::Hitbox() : m_id(-1), m_x(0), m_y(0), m_width(0), m_height(0) {}

Hitbox::Hitbox(const int &id, const int &x, const int &y, const int &w,
               const int &h)
    : m_id(id), m_x(x), m_y(y), m_width(w), m_height(h) {}
...
int Hitbox::getX() const { return m_x; }

// Return the y position of the Hitbox
int Hitbox::getY() const { return m_y; }
...
int Hitbox::setX(const int &x) {
  if (x < 0) THROW_VALUE(std::to_string(x));
...
int Hitbox::setY(const int &y) {
  if (y < 0) THROW_VALUE(std::to_string(y));
...
// Move to a position
int Hitbox::moveTo(const int &x, const int &y) { return setX(x) + setY(y); }