...
ManageHitbox::ManageHitbox() { m_idCount = 0; }
...
int ManageHitbox::getIdCount() const { return m_idCount; }
...
std::deque<Hitbox> ManageHitbox::getHitbox() const { return m_hitbox; }
...
std::deque<Hitbox> &ManageHitbox::getRealHitbox() { return m_hitbox; }

// Add a tile hitbox with checking the other layer
Hitbox &ManageHitbox::addTopTileHitbox(const int &tx, const int &ty,
                                       const Hitbox &hb) {
  auto it(m_tileHitbox.find(std::make_pair(tx, ty)));

  if (it == m_tileHitbox.end()) return addTileHitbox(tx, ty, hb);

  it->second.setWidth(MAX(it->second.getX() + it->second.getWidth(),
                          hb.getX() + hb.getWidth()) -
                      it->second.getX());
  it->second.setHeight(MAX(it->second.getY() + it->second.getHeight(),
...
// Add a tile hitbox
Hitbox &ManageHitbox::addTileHitbox(const int &tx, const int &ty,
                                    const Hitbox &hb) {
  if (tx < 0 || ty < 0)
    THROW_VALUE("Wrong tile hitbox position : " + std::to_string(tx) +
                std::to_string(ty));
...
Hitbox &ManageHitbox::addHitbox(const Hitbox &hb, const int &id) {
  ...
  if (id == -1) m_hitbox.back().setId(++m_idCount);
...
Hitbox &ManageHitbox::at(const int &tx, const int &ty) {
  ...
  if (it == m_tileHitbox.end())
    THROW_VALUE("TileHitbox doesn't exist : " + std::to_string(tx) +
                std::to_string(ty));
...
sf::Rect<int> ManageHitbox::rectTilePosition(const Hitbox &hb) {
  ...
  // Get the width
  for (rect.width = 1;
       (rect.left + rect.width) * TILE_WIDTH < hb.getX() + hb.getWidth();
       rect.width++)
    ;

  // Get the height
  for (rect.height = 1;
       (rect.top + rect.height) * TILE_HEIGHT < hb.getY() + hb.getHeight();
       rect.height++)
    ;
...
ManageHitbox::e_CollisionType ManageHitbox::willCollide(const Hitbox &hb,
                                                        const int &dx,
                                                        const int &dy) {
  Hitbox newHb(hb);
...
ManageHitbox::e_CollisionType ManageHitbox::areColliding(const Hitbox &hb1,
                                                         const Hitbox &hb2) {
