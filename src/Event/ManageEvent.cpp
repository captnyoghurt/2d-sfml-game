...
// Modify a key event
int ManageEvent::setKeyEvent(int layer, int code, unsigned short matchCode,
                             bool down) {
...
int ManageEvent::createLayers(const int &l) {
  ...
  if (n < l) {
    for (int i(n); i < l; i++) {
      m_keyEventList.at(i).resize(sf::Keyboard::Key::KeyCount,
                                  std::make_pair(0, 0));
    }
  }

  if (m_keyEventLayer < l) return 0;
  m_keyEventLayer = 0;
  return 1;
}
    ...
    switch (event.type) {
      ...
      case sf::Event::KeyPressed:
        if ((unsigned)event.key.code < sf::Keyboard::Key::KeyCount)
          m_matchingKeyEvent
              .at(m_keyEventList.at(m_keyEventLayer).at(event.key.code).first)
              .doAction(g);
        break;
      case sf::Event::KeyReleased:
        if ((unsigned)event.key.code < sf::Keyboard::Key::KeyCount)
          m_matchingKeyEvent
              .at(m_keyEventList.at(m_keyEventLayer).at(event.key.code).second)
              .doAction(g);
        break;
