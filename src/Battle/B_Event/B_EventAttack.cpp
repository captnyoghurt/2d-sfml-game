...
int B_EventAttack::execute(Battle *bat, ManageSound &snd) {
  ...
  if (m_enemyDestination.size() < (unsigned)m_numberEnemyDestination ||
    ...
    THROW_VALUE("Not enough destination " +
                std::to_string(m_enemyDestination.size()) + "/" +
                std::to_string(m_allyDestination.size()));
  ...
  if (m_enemyDestination.at(0)->getHealth().getPoints() <= 0) {
    if (!m_enemyDestination.at(0)->isTeamMate()) {
      std::vector<std::shared_ptr<Fighter> > &fighters =
          bat->getRealEnemies().getRealTeam();
      ...
      std::vector<std::shared_ptr<TeamMate> > &fighters =
          bat->getRealAllies().getRealTeam();
  ...
  if (m_sound != nullptr) snd.addSound(*m_sound);

  m_enemyDestination.at(0)->getRealHealth().use(
      MAX(1, m_source->getCharacteristics()
                     .at(Characteristic::e_characteristics::ATTACK_PHYSIC)
                     .getValue() -
                 m_enemyDestination.at(0)
                     ->getCharacteristics()
                     .at(Characteristic::e_characteristics::DEFENSE_PHYSIC)
                     .getValue()));
