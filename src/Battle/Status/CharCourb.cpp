...
CharCourb::CharCourb()
    : m_id(0), m_brut(0), m_simple(0), m_square(0), m_cube(0) {}

CharCourb::CharCourb(const int &brut, const int &simple, const int &square,
                     const int &cube)
    : m_id(0), m_brut(brut), m_simple(simple), m_square(square), m_cube(cube) {}
...
// Return the id
int CharCourb::getId() const { return m_id; }

// Return the brut coeff
double CharCourb::getBrut() const { return m_brut; }

// Return the simple coeff
double CharCourb::getSimple() const { return m_simple; }

// Return the square coeff
double CharCourb::getSquare() const { return m_square; }

// Return the cube coeff
double CharCourb::getCube() const { return m_cube; }

// Modify the id
int CharCourb::setId(const int &id) {
  if (id < 0) THROW_VALUE("Wrong id value : " + std::to_string(id));
...
double CharCourb::result(const int &level) const {
  if (level < 0) THROW_VALUE("Wrong level value : " + std::to_string(level));
