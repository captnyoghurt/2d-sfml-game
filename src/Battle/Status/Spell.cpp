...
Spell::Spell(int id, const std::string name, int damage, int allyDestination,
             int enemiDestination, int costHP, int costMP, int costTP,
             int chanceSleep, int chancePoison, int chanceParalysis,
             int chanceMute, int iconId, const std::string &desc)
    : m_id(id),
      m_name(name),
      m_damage(damage),
...
// Return the damage
int Spell::getDamage() const { return m_damage; }

// Return ally destination
int Spell::getAllyDestination() const { return m_allyDestination; }

// Return the enemi destination
int Spell::getEnemiDestination() const { return m_enemiDestination; }

// Return the hp cost
int Spell::getCostHP() const { return m_costHP; }

// Return the mp cost
int Spell::getCostMP() const { return m_costMP; }

// Return the tp cost
int Spell::getCostTP() const { return m_costTP; }

// Return the chance of sleep
int Spell::getChanceSleep() const { return m_chanceParalysis; }

// Return the chance of poison
int Spell::getChancePoison() const { return m_chancePoison; }

// Return the chance of paralysis
int Spell::getChanceParalysis() const { return m_chanceParalysis; }

// Return the chance of mute
int Spell::getChanceMute() const { return m_chanceMute; }

// Return the icon id
int Spell::getIconId() const { return m_iconId; }

// Return the battle effects
std::deque<BattleEffects> Spell::getEffects() const { return m_effects; }

// Return the description
std::string Spell::getDescription() const { return m_description; }

// Return the battle effects with modifying possibilities
std::deque<BattleEffects> &Spell::getRealEffects() { return m_effects; }

// Modify the id
int Spell::setId(const int &id) {
  if (id < 0) THROW_VALUE(std::to_string(id));
...
int Spell::setDamage(const int &d) {
  if (d < 0) THROW_VALUE(std::to_string(d));
...
int Spell::setAllyDestination(const int &ad) {
  if (ad < 0) THROW_VALUE(std::to_string(ad));
...
int Spell::setEnemiDestination(const int &ed) {
  if (ed < 0) THROW_VALUE(std::to_string(ed));
...
int Spell::setChanceSleep(const int &chance) {
  if (chance < 0 || chance > 100) THROW_VALUE(std::to_string(chance));
...
int Spell::setIconId(const int &id) {
  if (id < 0) THROW_VALUE("Wrong icon id on spell : " + std::to_string(id));
