...
bool Battle::getStarted() const { return m_started; }

// Return if the turn has begun
bool Battle::getInTurn() const { return m_inTurn; }

// Return if the choices for the turn has been made
bool Battle::getChoicesFinished() const { return m_choicesFinished; }

// Return if the battle is shown
bool Battle::getShown() const { return m_shown; }

// Return the number of turn since the begining of the battle
int Battle::getBattleTurn() const {
  if (!m_started) THROW_INIT("Battle not started");

  return m_battleTurn;
}

// Return the enemie team
Enemies Battle::getEnemies() const { return m_enemieTeam; }

// Return the allie team
TeamBattle Battle::getAllies() const { return *m_alliesTeam; }

// Return the selected TeamMate
TeamMate Battle::getSelectedTeamMate() const {
  if (!m_started) THROW_GAME("Select TeamMate without a started game");
  ...
  if (!m_battleOrder.at(m_battleEventCreated - 1)->isTeamMate())
    THROW_VALUE("No selected team mate, but selected fighter" +
                std::to_string(m_battleEventCreated - 1));

  return *std::dynamic_pointer_cast<TeamMate>(
      m_battleOrder.at(m_battleEventCreated - 1));
}

// Return the last key event layer
int Battle::getLastKeyEventLayer() const { return m_lastKeyEventLayer; }

// Return the number of battle event created
int Battle::getBattleEventCreated() const { return m_battleEventCreated; }

// Return the battle exit
Battle::e_BattleExit Battle::getBattleExit() const { return m_battleExit; }

// Return the enemie team with modifying possibilities
Enemies &Battle::getRealEnemies() { return m_enemieTeam; }

// Return the allie team with modifiying possibilities
TeamBattle &Battle::getRealAllies() { return *m_alliesTeam; }

// Return the battleMenu with modifying possibilities
MenuBattle &Battle::getRealBattleMenu() { return *m_battleMenu; }
...
int Battle::startTurn(Game *g) {
  if (!m_started || m_inTurn) THROW_INIT("Battle not started");
...
int Battle::start(const std::string &backgroundFilename, TeamBattle team,
                  std::string enemies, Game *g) {
  if (m_started) THROW_INIT("Battle already started");
  ...
  // Texture background
  m_texture =
      g->getRealRessourceManager(Game::e_ressourcesLayer::RESSOURCES_BATTLE)
          .addTexture();
  m_texture->loadFromFile(backgroundFilename);
  m_background = g->getRealSurfaceManager(LAYER_BATTLE_BEGIN)
  ...
  std::dynamic_pointer_cast<SurfaceSprite>(m_background->second)
      ->setScale(CAMERA_WIDTH / std::dynamic_pointer_cast<SurfaceSprite>(
                                    m_background->second)
                                    ->getGlobalBounds()
                                    .width,
                 CAMERA_HEIGHT / std::dynamic_pointer_cast<SurfaceSprite>(
                                     m_background->second)
                                     ->getGlobalBounds()
                                     .height);
  ...
  // Menus
  m_battleMenu = std::make_shared<MenuBattle>(
      this,
      g->getRealRessourceManager(Game::e_ressourcesLayer::RESSOURCES_MENU),
      g->getRealSurfaceManager(LAYER_MENU), BATTLE_KEY_EVENT_LAYER);

  // Load sides
  std::dynamic_pointer_cast<MenuBattleSide>(
      m_battleMenu->getRealMenus().at(MenuBattle::BM_RIGHT))
      ->loadWithAllies(
          g->getRealRessourceManager(Game::e_ressourcesLayer::RESSOURCES_MENU),
          g->getRealSurfaceManager(LAYER_MENU), *m_alliesTeam);

  EnemiBasic f;
  f.setCharacteristic(Characteristic(5, 10000),
  ...
  std::dynamic_pointer_cast<MenuBattleSide>(
      m_battleMenu->getRealMenus().at(MenuBattle::BM_LEFT))
      ->loadWithEnemies(
          g->getRealRessourceManager(Game::e_ressourcesLayer::RESSOURCES_MENU),
          g->getRealSurfaceManager(LAYER_MENU), m_enemieTeam);

  // Dialog box
  std::dynamic_pointer_cast<MenuDialogBox>(
      m_battleMenu->getRealMenus().at(MenuBattle::BM_DIALOG))
      ->addText("D�but du combat.\n");
  std::dynamic_pointer_cast<MenuDialogBox>(
      m_battleMenu->getRealMenus().at(MenuBattle::BM_DIALOG))
      ->continueText(
          g->getRealRessourceManager(Game::e_ressourcesLayer::RESSOURCES_MENU),
...
int Battle::update(Game *g) {
  // [TODO]
  if ((!m_updated && !m_battleMenu->getUpdated()) || !m_started) return 1;
...
int Battle::definedOrder(Game *g) {
  ...
  for (unsigned int i(1); i < m_battleOrder.size(); i++) {
    for (int j(i - 1); j >= 0; j--) {
      if (m_battleOrder.at(i)
              ->getRealCharacteristics()
              .at(Characteristic::e_characteristics::SPEED)
              .getValue() <= m_battleOrder.at(j)
                                 ->getRealCharacteristics()
                                 .at(Characteristic::e_characteristics::SPEED)
                                 .getValue()) {
...
int Battle::chooseBattleEvent(Game *g) {
  ...
  if (m_battleOrder.at(m_battleEventCreated)->isTeamMate()) {
    ...
    IG_Action actionSpell(std::bind(&af_openMenuSpells,
                                    *std::dynamic_pointer_cast<TeamMate>(
                                        m_battleOrder.at(m_battleEventCreated)),
                                    std::placeholders::_1));

    m_battleMenu->setIsBlocking(true);
    std::dynamic_pointer_cast<MenuChoice>(
        m_battleMenu->getRealMenus().at(MenuBattle::BM_CHOICE))
        ->getChoices()
        .at(2)
        ->setAction(actionSpell);
    ...
    m_battleEventManager.getRealBattleEvents().push_back(
        m_battleOrder.at(m_battleEventCreated)->chooseEvent(g));
  }
