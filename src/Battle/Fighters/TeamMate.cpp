...
// Return the courbs
std::vector<CharCourb> TeamMate::getCourbs() const { return m_courbs; }
...
std::vector<Spell> TeamMate::getAllAvailableSpells() const {
  ...
  for (std::pair<int, Spell> basicSpell : m_basicSpells) {
    if (m_level < basicSpell.first) return sp;
...
CharCourb TeamMate::getExpCourb() const { return m_expCourb; }

// Return the health courb
CharCourb TeamMate::getHealthCourb() const { return m_healthCourb; }

// Return the mana courb
CharCourb TeamMate::getManaCourb() const { return m_manaCourb; }

// Return the current experience
int TeamMate::getExperience() const { return m_experience; }

// Return the current level
int TeamMate::getLevel() const { return m_level; }

// Return the courbs with modifying possibilities
std::vector<CharCourb> &TeamMate::getRealCourb() { return m_courbs; }
...
int TeamMate::addExperience(const int &exp) {
  ...
  if (exp < 0) THROW_VALUE("Wrong value of exp : " + std::to_string(exp));
...
int TeamMate::reload(const bool &healing) {
  ...
  if (healing) m_health.setPoints(m_health.getMaxPoints());
  m_mana.setMaxPoints((int)m_manaCourb.result(m_level));
  if (healing) m_mana.setPoints(m_mana.getMaxPoints());
...
bool TeamMate::isTeamMate() const { return true; }
