...
TeamBattle::TeamBattle() { m_numberTeamMateAlive = 0; }
...
int TeamBattle::getNumberTeamMateAlive() const {
  ...
  for (unsigned int i(0); i < m_team.size(); i++) {
    if (m_team.at(i)->getHealth().getPoints() > 0) n++;
  }
...
int TeamBattle::load(Game* g) {
  ...
  m_team.at(1)->setSurface(g->getRealSurfaceManager(LAYER_BATTLE_END)
                               .addSurface(ManageSurfaces::e_thing::SPRITE,
                                           std::make_shared<SurfaceSprite>()));
  std::dynamic_pointer_cast<SurfaceSprite>(
      m_team.at(1)->getRealSurface()->second)
      ->setTexture(*texture2);
  m_team.at(1)->getRealSurface()->second->setDimensions(
      TEAM_BATTLE_CHAR2_X, TEAM_BATTLE_CHAR2_Y,
      (int)std::dynamic_pointer_cast<SurfaceSprite>(
          m_team.at(1)->getRealSurface()->second)
          ->getGlobalBounds()
          .width,
