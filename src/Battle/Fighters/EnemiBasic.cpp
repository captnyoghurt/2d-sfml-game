...
std::shared_ptr<B_Event> EnemiBasic::chooseEvent(Game *g) {
  std::uniform_int_distribution<int> distribution(
      1, g->getRealBattle().getAllies().getNumberTeamMateAlive());
  int n(distribution(m_generator));

  for (unsigned int i(0); i < g->getRealBattle().getAllies().getTeam().size();
       i++) {
    if (!g->getRealBattle().getAllies().getTeam().at(i)->isDead()) n--;

    if (n == 0) {
      B_EventAttack evt(this->getName() + " attaque !");
      evt.getRealEnemyDestination().push_back(
          g->getRealBattle().getRealAllies().getRealTeam().at(i));
