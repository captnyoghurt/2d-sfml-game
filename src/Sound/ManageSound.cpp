...
int ManageSound::addMusic(const std::string &filename, bool loop) {
  if (!m_musics.empty()) m_musics.top()->stop();
...
int ManageSound::deleteMusic() {
  if (m_musics.empty()) THROW_GAME("Empty stack of music");
  ...
  if (m_musics.empty()) return 1;

  if (played == sf::Music::Status::Playing) m_musics.top()->play();
...
int ManageSound::update() {
  ...
  if (m_clock.getElapsedTime().asMilliseconds() < 1000) return 0;
...
int ManageSound::clear() {
  ...
  while (!m_musics.empty()) m_musics.pop();
