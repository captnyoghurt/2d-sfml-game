...
int initiate_constants(std::string filename) {
  ...
  if (!file.is_open()) THROW_RESSOURCE("Constant file", filename);
  ...
  while (std::getline(file, line)) {
    if (line.size() > 0 && line[0] != '#') {
      ...
      if (word == "WINDOW_WIDTH") {
        ...
        iss >> value;
        TEAM_WALK_ANIMATION_REFRESH =
            ((value >= 0) ? value : TEAM_WALK_ANIMATION_REFRESH);
        ...
        iss >> value;
        TEAM_WALK_FRAME_NUMBER =
            ((value >= 0) ? value : TEAM_WALK_FRAME_NUMBER);
      } else if (word == "TEAM_WALK_UP_POSITION_Y") {
        iss >> value;
        TEAM_WALK_UP_POSITION_Y =
            ((value >= 0) ? value : TEAM_WALK_UP_POSITION_Y);
      } else if (word == "TEAM_WALK_DOWN_POSITION_Y") {
        iss >> value;
        TEAM_WALK_DOWN_POSITION_Y =
            ((value >= 0) ? value : TEAM_WALK_DOWN_POSITION_Y);
      } else if (word == "TEAM_WALK_RIGHT_POSITION_Y") {
        iss >> value;
        TEAM_WALK_RIGHT_POSITION_Y =
            ((value >= 0) ? value : TEAM_WALK_RIGHT_POSITION_Y);
      } else if (word == "TEAM_WALK_LEFT_POSITION_Y") {
        iss >> value;
        TEAM_WALK_LEFT_POSITION_Y =
            ((value >= 0) ? value : TEAM_WALK_LEFT_POSITION_Y);
      } else {
...
int initiate_events(std::string filename, Game &g) {
  ...
  if (!file.is_open()) THROW_RESSOURCE("Events file", filename);

  file >> nb;

  if (nb <= 0) THROW_VALUE("Number of event layer incorrect");
  ...
  while (std::getline(file, line)) {
    if (line.size() > 0 && line[0] != '#') {
      ...
      if ((unsigned)layer < (unsigned)nb) {
        int code(0);
        if (iss >> word) code = getCodeFromWord(word);
        if (iss >> word) {
          if (word == "new") {
            iss >> word;
            IG_Action::s_action act = getActionFromWord(word);
            g.getRealEventManager().setKeyEvent(
                layer, code,
                g.getRealEventManager().createMatchingKey(IG_Action(act)),
                (pressed == 1) ? true : false);
          } else {
            g.getRealEventManager().setKeyEvent(layer, code, std::stoi(word));
          }
        }
      } else {
        THROW_VALUE("Impossible value of layer event : " +
                    std::to_string(layer));
      }
    }
  }
